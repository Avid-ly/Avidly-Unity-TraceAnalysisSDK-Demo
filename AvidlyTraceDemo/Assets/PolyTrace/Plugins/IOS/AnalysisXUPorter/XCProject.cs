	...
	public partial class XCProject : System.IDisposable
	{
		...
		public bool AddFolder( string folderPath, PBXGroup parent = null, string[] exclude = null, bool recursive = true, bool createBuildFile = true )
		{
			...
			foreach( string file in Directory.GetFiles( folderPath ) ) {
				// Kanglai: ignore "" case
				if( regexExclude.Length > 0 && Regex.IsMatch( file, regexExclude ) ) {
					continue;
				}
		...
		private string combinePathWithCheck(string rootPath, string filePath) {
			string absoluteFilePath = System.IO.Path.Combine(rootPath, filePath);
			Debug.Log("combinePathWithCheck rootPath: " + rootPath);
			Debug.Log("combinePathWithCheck absolutePath: " + absoluteFilePath);

			if (absoluteFilePath.Equals (filePath)) {
				Debug.Log("combinePathWithCheck failed, will appe again ");
				if (!rootPath.EndsWith ("/")) {
					...
					if (!filePath.StartsWith ("/")) {
						...
						absoluteFilePath = rootPath + filePath.Substring(1);
					}
				}
				Debug.Log("combinePathWithCheck absolutepath2: " + absoluteFilePath);
			}
